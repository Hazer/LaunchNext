...
// Note: `GestureTriggerAction`, `GestureTouchSample`, and `GestureTouchFrame`
// are declared in GestureTypes.swift. This file is a near-verbatim port of
// worktree-agent-afad21ec's LaunchNext/Gesture/GestureStateMachine.swift,
// relocated to LaunchNextInput/Gesture/ and adapted to consume the
// OMS-free GestureTouchSample type.

/// 5-state per-finger multitouch gesture detector.
///
/// Consumes a stream of `GestureTouchFrame`s (one per HID event) and emits
/// `GestureTriggerAction`s when an open/close/tap gesture is recognized.
/// Designed for use with a per-device state machine map keyed by
/// `GestureTouchFrame.deviceID`.
///
/// State flow: `idle → arming → tracking → triggered → cooldown → idle`.
/// See `docs/gesture-investigation.md` for the full design rationale and
/// comparison with the simpler scale-ratio detector in HIDGestureMonitor.
public struct GestureStateMachine {
    ...
    public mutating func consume(samples: [GestureTouchSample], at time: TimeInterval = ProcessInfo.processInfo.systemUptime) -> GestureTriggerAction? {
